...
const ICONS = [
  ...
  <svg key="3" className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>,
...
export default function Education() {
  return (
    <section className="py-24 lg:py-32 bg-white" id="education">
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        <FadeIn className="text-center mb-16">
          <span className="section-label text-primary/60 mb-3 block">Education</span>
        ...
        <FadeIn>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {EDUCATION_DEPTS.map((dept, i) => (
              <a
                key={dept.name}
                href="https://edu.gracecc.or.kr"
                className="card-hover group flex items-center gap-4 bg-surface rounded-2xl px-6 py-5 border border-gray-100"
              >
                <div className="w-10 h-10 rounded-xl bg-primary/[0.06] text-primary/60 flex items-center justify-center shrink-0 group-hover:bg-primary group-hover:text-white transition-all duration-300">
                  {ICONS[i]}
                </div>
                <div>
                  <h4 className="font-semibold text-[14px] text-gray-900 group-hover:text-primary transition-colors">
                    {dept.name}
                  </h4>
                  <p className="text-[12px] text-gray-400">{dept.desc}</p>
                </div>
              </a>
            ))}
          </div>
